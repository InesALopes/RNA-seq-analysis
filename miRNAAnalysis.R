...
write.table(down, file="down_regulatedeverything_M1M2.txt", sep="\t")
writeLines(rownames(down), con="down_regulatedgenes_M1M2.txt")

#up-regulated
preup <- subset(resSig, log2FoldChange>= 1)
up<-preup[order(preup$log2FoldChange, decreasing = TRUE),]

write.table(up, file="up_regulatedeverything_M1M2.txt", sep="\t")
writeLines(rownames(up), con="up_regulatedgenes_M1M2.txt")
